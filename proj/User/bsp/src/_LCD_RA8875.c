...
void RA8875_InitHard(void)
{
	...
	*/
	...
	else	/* GPIX = 0 表示7寸屏(800x480) */
	{
		...
		//g_LcdHeight = LCD_70_WIDTH;
		//g_LcdWidth = LCD_70_HEIGHT;
...
void RA8875_DrawHLine(uint16_t _usX1 , uint16_t _usY1 , uint16_t _usX2 , uint16_t _usColor)
{
	...
	if (g_LcdDirection > 1)	/* 竖屏  */
	{
		RA8875_REG = 0x46; RA8875_RAM = _usY1;
		RA8875_REG = 0x47; RA8875_RAM = _usY1 >> 8;
		RA8875_REG = 0x48; RA8875_RAM = _usX1;
		RA8875_REG = 0x49; RA8875_RAM = _usX1 >> 8;	
	}
	else	/* 横屏 */
	{
...
#else	
	RA8875_DrawLine(_usX1 , _usY1 , _usX2 , _usY1 , _usColor);
#endif
...
void RA8875_DrawVLine(uint16_t _usX1 , uint16_t _usY1 , uint16_t _usY2 , uint16_t _usColor)
{
	...
	{	
		...
		if (g_LcdDirection > 1)	/* 竖屏  */
		{
			RA8875_REG = 0x46; RA8875_RAM = _usY1 + i;
			RA8875_REG = 0x47; RA8875_RAM = ( _usY1 + i) >> 8;
			RA8875_REG = 0x48; RA8875_RAM = _usX1;
			RA8875_REG = 0x49; RA8875_RAM = _usX1 >> 8;
		}
		else	/* 横屏 */
...
#else	
	RA8875_DrawLine(_usX1 , _usY1 , _usX1 , _usY2 , _usColor);
#endif
