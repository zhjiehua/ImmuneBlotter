...
//#define DEBUG( format, args... )  printf( format, ##args ) //C++11规范
#define DEBUG( format, ... )   printf( format, ##__VA_ARGS__ ) //C99规范
...
typedef struct WIN_PARA{			//窗口使用到的用户定义参数，方便在回调函数中使用
	...
	int ySizeWinStatus, ySizeWinCtrl;//状态窗口和控制窗口的高度
...
/***图标**/
extern GUI_CONST_STORAGE GUI_BITMAP bmJoinmax_Logo;         //中鸣Logo，未按下状态
