      ...
      //
 	  //LCD_PutPixel(xPhys, yPhys, PixelIndex);
		if (g_ChipID == IC_8875)
		{
			...
			//RA8875_PutPixel(xPhys, yPhys, PixelIndex);
		}
		else
		{
      ...
      //
 	  //PixelIndex = LCD_GetPixel(xPhys, yPhys);
		if (g_ChipID == IC_8875)
		{
			...
			//PixelIndex = RA8875_GetPixel(xPhys, yPhys);
		}
...
//  LCD_PIXELINDEX ColorIndex;
  if (GUI_pContext->DrawMode & LCD_DRAWMODE_XOR) {
    for (; x0 <= x1; x0++) {
	  ...
	  #if emWin_Optimize
		//emWin_DrawHLine(x0, y, x1, LCD_COLORINDEX);
        if (g_ChipID == IC_8875)
		{
			RA8875_DrawHLine( x0, y, x1, LCD_COLORINDEX);
		}
		else
		{
			ILI9488_DrawHLine(x0, y, x1, LCD_COLORINDEX);
		}
	  #else
		LCD_PIXELINDEX ColorIndex;
...
//  LCD_PIXELINDEX ColorIndex;
  ...
  if (GUI_pContext->DrawMode & LCD_DRAWMODE_XOR) {
    ...
    #if emWin_Optimize
		//emWin_DrawVLine(x, y0, y1, LCD_COLORINDEX);
		if (g_ChipID == IC_8875)
		{
			 RA8875_DrawVLine(x, y0, y1, LCD_COLORINDEX);
		}
		else
		{
			ILI9488_DrawVLine(x, y0, y1, LCD_COLORINDEX);
		}
	#else
		LCD_PIXELINDEX ColorIndex;
		ColorIndex = LCD__GetColorIndex();
...
//		}
		RA8875_RTERect( x0,  y0,  x1,  y1,  LCD_COLORINDEX);
		//RA8875_FillRect(x0, y0, (y1-y0+1), (x1-x0+1), LCD_COLORINDEX);		
	}
	else
	{
		ILI9488_FillRect(x0, y0, y1-y0+1, x1-x0+1, LCD_COLORINDEX);
	}
#else
	for (; y0 <= y1; y0++) 
	{
...
static void _DrawBitmap(GUI_DEVICE * pDevice, int x0, int y0,
  ...
  case 16:
    for (i = 0; i < ySize; i++) {
     ...
     //RA8875_DrawBMP(x0, y0, ySize, xSize, (uint16_t *)pData);
    break;
  case 32:
    for (i = 0; i < ySize; i++) {
